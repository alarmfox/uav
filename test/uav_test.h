...
#define TEST_ASSERT_EQ(expected, actual)                                       \
  do {                                                                         \
    ...
    if (uav_expected_ != uav_actual_) {                                        \
      printf("    %s:%d: expected %" PRIdMAX ", got %" PRIdMAX "\n", __FILE__, \
             __LINE__, uav_expected_, uav_actual_);                            \
      return 1;                                                                \
    }                                                                          \
  } while (0)
  ...
  do {                                                               \
    ...
    if (uav_expected_ != uav_actual_) {                              \
      printf("    %s:%d: expected %p, got %p\n", __FILE__, __LINE__, \
             uav_expected_, uav_actual_);                            \
      return 1;                                                      \
    }                                                                \
  } while (0)

#define TEST_ASSERT_NULL(pointer) TEST_ASSERT_PTR_EQ(NULL, (pointer))
